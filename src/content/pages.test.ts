...
describe('the registry', () => {
  ...
  it('gives every page a headline the e2e suite can anchor on', () => {
    // e2e asserts the rendered h1 contains `headline`, so it has to be real
    // text rather than a placeholder. It is deliberately NOT required to be a
    // substring of the title: the title is written for a search result and the
    // h1 for the reader, and on this site the home page's h1 is the tagline
    // while its title leads with what the tool does.
    for (const page of SITE_PAGES) {
      expect(page.headline.trim().length, `${page.path} has no headline`).toBeGreaterThan(8)
      expect(page.headline).toBe(page.headline.trim())
    }
  })
})
