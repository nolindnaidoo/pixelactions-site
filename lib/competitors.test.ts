...
import { COMPETITORS, PIXELACTIONS_CELLS, ROW_KEYS } from './competitors'
...
describe('competitor quarantine data', () => {
  ...
  it('fills every row for every column, ours included', () => {
    for (const key of ROW_KEYS) {
      expect(PIXELACTIONS_CELLS[key].value.length).toBeGreaterThan(0)
      for (const competitor of COMPETITORS) {
  ...
  it('pixelactions does not sweep the table — the policy binds both ways', () => {
    const losses = ROW_KEYS.filter(key => PIXELACTIONS_CELLS[key].wins !== true)
