...
import { TOOL_NAME } from '../src/content/site'

// The hardening gates: keyboard, reflow, theme, 404, SEO
...
test.describe('keyboard', () => {
  ...
  test('a FAQ summary opens with Enter', async ({ page }) => {
    const howTo = SITE_PAGES.find(page => page.path.startsWith('/how-to/'))?.path ?? '/'
...
test.describe('seo furniture', () => {
  ...
  test('manifest, security.txt, and llms.txt are served', async ({ request }) => {
    ...
    expect((await manifest.json()).name).toBe(TOOL_NAME)
