...
// The hardening gates: keyboard, reflow, theme, 404, SEO furniture.
// These run alongside the per-page axe specs.
...
test.describe('keyboard', () => {
  ...
  test('a FAQ summary opens with Enter', async ({ page }) => {
    await page.goto('/how-to/automate-desktop-clicks')
...
test.describe('seo furniture', () => {
  ...
  test('manifest, security.txt, and llms.txt are served', async ({ request }) => {
    ...
    expect((await manifest.json()).name).toBe('pixelactions')
